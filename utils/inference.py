...
def draw_landmarks(img, pts, style='simple', wfp=None, show_flg=False, **kwargs):
    ...
    for i in range(len(pts)):
        if style == 'simple':
            ...
            for _i in range(len(pts[i][0,:])):
                plt.plot(pts[i][0, _i], pts[i][1, _i], 'o', markersize=4, color='g')
                plt.annotate(str(_i+1), xy= (pts[i][0, _i], pts[i][1,_i]))
